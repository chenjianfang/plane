...
var scores=0;
    ...
    //屏幕可见高度
var my_height = document.body.clientHeight; 
    //屏幕可见宽度
var my_width = document.body.clientWidth;
/*
 创建飞机类
 */
function plan(hp,X,Y,sizeX,sizeY,score,dietime,sudu,boomimage,imagesrc){  //cjf:sizeX,sizeY飞机的宽高，boomimage飞机爆炸图片，imagesrc飞机没爆照图片 
    this.planX=X;
    this.planY=Y;
    this.imagenode=null;  //cjf:出现新加的飞机
    this.planhp=hp;
...
移动行为
     */
    this.planmove=function(){      //cjf:分数区间不同的移动速度
        if(scores<=50000){
...
创建子弹类
 */
function bullet(X,Y,sizeX,sizeY,imagesrc){    //X，Y看不出来是什么  sizeX,sizeY子弹的大小，imagesrc子弹链接的图片
    this.bulletX=X;
    this.bulletY=Y;
...
var selfplan=new ourplan(120,285);
//移动事件
var ourPlan=document.getElementById('ourplan');
var yidong=function(event){
    var oevent=event.targetTouches[0];
...
var bianjie=function(event){
    ...
    if(bodyobjX<5||bodyobjX>my_width-5||bodyobjY<0||bodyobjY>my_height){
        if(document.removeEventListener){
            mainDiv.removeEventListener("touchmove",yidong,true);
        }
    }
...
//    suspenddiv.getElementsByTagName("button")[1].addEventListener("click",chongxinkaishi,true);  //cjf:重新开始函数没写
    //为暂停界面的返回主页按钮添加事件
...
function start(){
    ...
    if(backgroundPositionY==my_height){      //cjf:循环显示背景
    ...
    if(mark==20){
        mark1++;
        //中飞机
        if(mark1%5==0){         //第一个是打几个子弹会爆炸，第二、三个是飞机出现的X轴的区间，4/5是飞机的宽高、6是打爆此类飞机的得分，7死亡时间
            enemys.push(new enemy(6,25,my_width,46,60,5000,360,random(1,3),"image/中飞机爆炸.gif","image/enemy3_fly_1.png")); 
        }
        //大飞机
        if(mark1==20){
            enemys.push(new enemy(12,50,200,110,164,30000,540,1,"image/大飞机爆炸.gif","image/enemy2_fly_1.png"));
            mark1=0;
        }
        //小飞机
        else{
            enemys.push(new enemy(1,0,my_width,34,24,1000,360,random(1,4),"image/小飞机爆炸.gif","image/enemy1_fly_1.png"));
        }
        mark=0;
    }
...
/*
 如果敌机超出边界,删除敌机
 */
        if(enemys[i].imagenode.offsetTop>my_height){
